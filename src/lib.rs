...
impl egg::Analysis<PyLang> for PyAnalysis {
    ...
    fn make(egraph: &egg::EGraph<PyLang, Self>, enode: &PyLang) -> Self::Data {
        ...
        Some(res)
        // let py = unsafe { Python::assume_gil_acquired() };
        // let an = &egraph.analysis.eval;
        // if an.is_none(py) {
        //     return py.None();
        // }
        // let args = (enode.to_object(py, |id),);
        // match an.call_method1(py, "make", args) {
        //     Ok(res) => res,
        //     Err(err) if err.is_instance::<PyNotImplementedError>(py) => py.None(),
        //     Err(err) => panic!("Analysis::make failed on {} with {}", enode, err),
        // }
    }
